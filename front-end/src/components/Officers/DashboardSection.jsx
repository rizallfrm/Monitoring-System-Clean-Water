...
export function DashboardSection() {
      ...
      icon: (
        ...
        >
          <path
            ...
            d="M9 12h6M9 16h6M4 6h16M4 6v14a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V6M4 6V4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v2"
          />
        </svg>
      ),
    },
      ...
      icon: (
        ...
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M18.364 5.636L5.636 18.364M12 21c4.9706 0 9-4.0294 9-9s-4.0294-9-9-9-9 4.0294-9 9 4.0294 9 9 9z"
          />
        </svg>
      ),
    },
  ...
  useEffect(() => {
    // Di DashboardSection.jsx sebelum fetchStats
    console.log("ReportService:", reportService);
    console.log("StatusService:", statusService);
    console.log("API function:", typeof api);
    const fetchStats = async () => {
      try {
        setLoading(true);
        setError(null);

        // Hanya ambil data reports jika endpoint statistics tidak ada
        const reportsResponse = await reportService.getAllReports();
        const reports = reportsResponse.data || reportsResponse;

        // Hitung manual jika endpoint statistics tidak ada
        const statusStats = {
          menunggu: reports.filter(
            (r) => r.status === "Pending" || r.status === "Menunggu"
          ).length,
          selesai: reports.filter(
            (r) =>
              r.status === "Selesai" ||
              r.status === "Complete" ||
              r.status === "Resolved"
          ).length,
          dibatalkan: reports.filter(
            (r) => r.status === "Dibatalkan" || r.status === "Cancelled"
          ).length,
        };

        setStats([
          { ...stats[0], value: reports.length.toString() },
          { ...stats[1], value: statusStats.menunggu.toString() },
          { ...stats[2], value: statusStats.selesai.toString() },
          { ...stats[3], value: statusStats.dibatalkan.toString() },
        ]);
      } catch (error) {
