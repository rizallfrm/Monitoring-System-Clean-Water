...
import { 
  Home, FileText, CheckCircle, LogOut, Activity 
} from 'lucide-react';
    ...
    <div className="fixed inset-y-0 left-0 w-64 bg-gradient-to-b from-white via-gray-50/50 to-white shadow-2xl border-r border-gray-200 flex flex-col overflow-hidden">
      ...
      <div className="p-6 border-b border-gray-200 relative z-10">
        <div className="flex items-center group">
          ...
          <div className="ml-4">
            ...
            <p className="text-sm text-gray-500 mt-0.5">Kelola laporan & tindakan</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2 relative z-10">
        {[
          ...
          { id: 'reports', label: 'Laporan', icon: FileText, gradient: 'from-green-500 to-emerald-600' },
          { id: 'actions', label: 'Tindakan', icon: CheckCircle, gradient: 'from-purple-500 to-pink-600' }
        ].map(tab => (
          <button
            key={tab.id}
            ...
            className={`group flex items-center w-full px-4 py-4 text-sm font-semibold rounded-2xl transition-all duration-300 relative overflow-hidden ${
              activeTab === tab.id
                ? 'bg-gradient-to-r text-white shadow-lg transform scale-105 ' + tab.gradient
          ...
          >
            ...
            <div className={`p-2 rounded-xl mr-3 transition-all duration-300 relative z-10 ${
              activeTab === tab.id 
                ? 'bg-white/20' 
        ...
        <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl p-4 border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-3 flex-1">
              <p className="text-sm font-bold text-gray-900">Joko Purnomo</p>
              <p className="text-xs text-gray-500">Petugas Lapangan</p>
            </div>
          </div>
          <button className="flex items-center justify-center w-full mt-3 px-3 py-2 text-xs text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all duration-200 font-medium border border-gray-200 hover:border-red-200">
