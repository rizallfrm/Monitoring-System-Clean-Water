...
const statusController = {
      ...
      if (req.user.role === 'Warga') {
        if (report.user_id !== userId) {
          return res.status(403).json({
            status: 'error',
            message: 'Anda tidak memiliki akses untuk mengubah status laporan ini'
          });
        }
  ...
  getAllStatusUpdates: async (req, res) => {
    try {
      const { reportId, userId, status, startDate, endDate, page = 1, limit = 10 } = req.query;
  ...
  getStatusStatistics: async (req, res) => {
    try {
      ...
      return res.status(500).json({
        status: 'error',
        message: 'Terjadi kesalahan saat mengambil statistik status',
        error: error.message
      });
    }
  }
