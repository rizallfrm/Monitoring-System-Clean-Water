...
const actionController = {
  ...
  getAllActions: async (req, res) => {
    try {
      const { reportId, performedBy, startDate, endDate, page =.1, limit = 10 } = req.query;
      ...
      const action = await Action.findByPk(id, {
        include: [
          ...
          },
          {
            model: Report,
            as: 'report',
            attributes: ['report_id', 'description', 'location', 'status', 'user_id']
          }
        ]
      });
  ...
  updateAction: async (req, res) => {
    try {
      ...
      if (action.report.status === 'Completed' || action.report.status === 'Cancel') {
        return res.status(400).json({
          status: 'error',
